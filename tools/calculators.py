...
def analyze_hill_response(recompute: bool = False) -> dict:
    """Fit the athlete's grade→pace response from their own run history.

    Derives how much the runner slows on hills at easy effort — a fitted
    ...
    feed `analyze_race_course(grade_model='fitted')` for athlete-calibrated course
    ...
    fit at easy effort and used as an individualized GAP (shape transfers; absolute
...
def analyze_race_course(
    ...
    Args:
        ...
        grade_model: 'heuristic' (default) uses the generic Minetti-family
            grade adjustment; 'fitted' uses the athlete's OWN fitted
            ...
            'fitted' auto-fits+caches on first use; falls back to heuristic
    ...
    if goal_time_s is not None:
        ...
        # curve (analyze_hill_response). 'fitted' auto-computes+caches on first
        # use; falls back to heuristic with a note if it can't be fit.
        grade_fn = None
        grade_model_note = None
        if grade_model == "fitted":
            import hill_model
            m = hill_model.load_grade_response() or hill_model.compute_grade_response()
            if "error" in m:
                grade_model_note = (
                    "Requested fitted grade model but it couldn't be fit ("
                    + m["error"] + ") — used the generic heuristic instead."
                )
            else:
                grade_fn = lambda g: hill_model.fitted_grade_factor(g, m)
                grade_model_note = (
                    f"Using your fitted grade curve (fit from {m['n_runs']} easy "
        ...
        if "error" in plan:
            return plan
        plan["grade_model"] = "fitted" if grade_fn else "heuristic"
        if grade_model_note:
