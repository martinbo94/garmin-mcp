...
| `analyze_race_course` | GPX course analysis (sustained climbs/descents + short steep "wall" pitches) + grade-adjusted per-km race pacing for a goal (even-effort default, optional conservative-start negative-split; `grade_model='fitted'` uses your own fitted hill curve) |
| `analyze_hill_response` | Fit a grade→pace curve from your own run history (how much you slow per 1% incline at easy effort); feeds `analyze_race_course` |
