"""Fitted grade→pace response, derived from the athlete's own run streams.

Empirically derives how much THIS runner slows on hills at easy effort, so a
race-course pace plan can use an athlete-specific grade-adjustment curve instead of a
...
The factor is derived at EASY effort but used as an individualized GAP: the *shape*
...
Fit is cached to coach_data/grade_model.json (per-athlete artifact; gitignored).
"""
import json
...
def compute_grade_response(recompute: bool = False) -> dict:
    """Fit (or load cached) the athlete's grade→pace response. See module docs."""
    if not recompute:
    ...
    if len(recs) < 200 or n_runs < 10:
        return {"error": (
            f"Not enough steady easy-run stream data to fit a grade "
    ...
    model = {
        ...
        "caveats": (
            "Derived at easy effort (used as an individualized GAP — shape transfers, "
...
def fitted_grade_factor(grade_pct: float, model: dict) -> float:
